...
- **Core library** (`@agentdoor/core`): Ed25519 challenge-response auth, typed error hierarchy, agent storage interface, rate limiting, JWT issuance, x402 payment support.
- **Framework adapters**: Express.js, Next.js (App Router), Hono, Fastify, FastAPI (Python), Cloudflare Workers, Vercel Edge.
- **Agent SDKs**: TypeScript (`@agentdoor/sdk`) and Python (`agentdoor`) with automatic discovery, registration, credential caching, and token refresh.
- **CLI tool** (`@agentdoor/cli`): `npx agentdoor init` with OpenAPI import support.
- **Agent traffic detection** (`@agentdoor/detect`): Classify requests as human or agent based on headers, user-agent, and behavioral patterns.
